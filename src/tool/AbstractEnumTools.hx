...
class AbstractEnumTools 
{
	...
	public static macro function getValues(typePath:Expr):Expr {
		return _getValues(typePath);
	}
	
	public static macro function map(typePath:Expr, mapExpr:Expr):Expr 
	{
		var values = _getValues(typePath);
		var cases = [];
		switch (mapExpr.expr)
		{
			case ExprDef.EArrayDecl(exprs):
				for (expr in exprs)
				{
					switch (expr.expr)
					{
						case ExprDef.EBinop(Binop.OpArrow, e1, e2):
							cases.push({
								values: [e1],
								expr: e2,
							});
						...
						default:
							throw new Error(expr.toString() + " should be map declaration", expr.pos);
					}
				}
				
			default:
				throw new Error(mapExpr.toString() + " should be array declaration", mapExpr.pos);
		}
		var swichExpr = {
			expr: ExprDef.ESwitch(macro @:pos(mapExpr.pos) __id, cases, null),
			pos: mapExpr.pos
		}
		return macro @:pos(mapExpr.pos) [for (__id in $values) __id => $swichExpr];
	}
	
	#if macro
	// https://code.haxe.org/category/macros/enum-abstract-values.html	
	public static function _getValues(typePath:Expr):Expr {
